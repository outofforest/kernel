...
//
// However, to apply a handler to all requests handled by the server, which is
// the most common use case, it's convenient to pass the thttp.Middleware option
// to thttp.NewServer. The option takes any number of middleware, which are
// applied in order so that the first one listed is the first to see the
// incoming request.
//
...
// been sent), without the details of the error being exposed.
package thttp
